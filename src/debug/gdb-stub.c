...
extern void exceptionHandler(int exception_number, void *exception_address);	/* assign an exception handler   */
	  ...
	  if (checksum != xmitcsum)
	    {
	      if (remote_debug)
		{
          print("bad checksum. My count= 0x");
          printhex(checksum);
          print(", sent= 0x");
          printhex(xmitcsum);
          print(", buf= 0x");
...
handle_exception (int exceptionVector)
{
  ...
  if (remote_debug)
    {
      print("vector=");
      printint(exceptionVector);
      print(", sr=0x");
      printhex(registers[PS]);
      print(", pc=0x");
      printhex(registers[PC]);
